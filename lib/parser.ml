...
and declaration (tokens : Tokens.t list) : Tokens.t list * Ast.stmt_t =
  ...
  | { token_type = Tokens.CLASS; _ }
    ...
    :: rest ->
      class_stmt name_token
        (Some { token = superclass_token; distance = ref None })
        rest
  | { token_type = Tokens.CLASS; _ }
    :: ({ token_type = Tokens.IDENTIFIER; _ } as name_token)
    :: rest ->
